...
export class Board {
 ...
 public:
  Board(istream& boardIn, istream& squaresIn) {
    ...
    while (getline(squaresIn, line)) {
      ...
      if (squareType == "AcademicBuilding") {
        ...
        squareptr = make_unique<AcademicBuilding>(
            squareName, squarePosition, squareCost, squareImprovementCost,
        ...
        boardString.replace(location + 2 * lineWidth, squareName.length(),
                            squareName);
      ...
      for (auto& squareptr :
           squares) {  // set blockSize of all academic buildings
        if (auto acBuild = dynamic_cast<AcademicBuilding*>(squareptr.get())) {
          acBuild->setBlockSize(
              monopolyBlockCounts[acBuild->getMonopolyBlock()]);
        }
      }
    }
  }
  void display(ostream& out = std::cout) {
    ...
    for (auto& squareptr : squares) {  // display improvements
      if (auto acBuild = dynamic_cast<AcademicBuilding*>(squareptr.get())) {
        ...
        boardString.replace(location + lineWidth, numImprovements,
                            improvementsString);
      }
    }
    int playerPosition;
    char playerToken;
    int playerOffset =
        0;  // ensures players can't be printed on top of each other
